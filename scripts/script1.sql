...
CREATE TABLE IF NOT EXISTS idees (
  ...
  vote INT DEFAULT 0,
  date_de_creation TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_idees_utilisateurs 
...
CREATE TABLE IF NOT EXISTS commentaires (
  ...
  CONSTRAINT fk_commentaires_utilisateurs 
    ...
    REFERENCES utilisateurs(id) 
    ON DELETE CASCADE,
  CONSTRAINT fk_commentaires_idees 
    FOREIGN KEY (id_idee) 
    REFERENCES idees(id) 
    ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS votes (
  ...
  id_idee INT NOT NULL,
  date_de_creation TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_votes_utilisateurs 
    FOREIGN KEY (id_utilisateurs) 
    REFERENCES utilisateurs(id) 
    ON DELETE CASCADE,
  CONSTRAINT fk_votes_idees 
    FOREIGN KEY (id_idee) 
    REFERENCES idees(id) 
    ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS idees_categories (
  ...
  CONSTRAINT fk_idees_categories_idees 
    FOREIGN KEY (id_idee) 
    REFERENCES idees(id) 
    ON DELETE CASCADE,
  CONSTRAINT fk_idees_categories_categories 
